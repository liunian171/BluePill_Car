 ...
 *    3) 真的"不阻塞"：真实实现每笔最坏 100ms（UART_TIMEOUT_MS，
 *       2026-09-20 对齐原直调口径），空壳为 0 —— 对控制节拍只会更安全
 *
 *  ▸ 与组装层接线的关系（两层缺一不可）◂
 *    本文件只管"平台 ops 通道"；主循环的 UART 中断接收与直接发送在组装层
 *    main.c 里由 LINK_UART_ENABLED 门控（见 main.c [P3 双开关] 段）。
 *    两者都关掉才算 UART 链路真正下线。
 *    [P1-1 补回该层 2026-09-20] 组装层收发已改经策略层 → 本 stub 由
 *    "符号级空壳"升级为真实门控链的一环（ISR 重挂经 receive_IT 也会落到这）。
 *
 *  ▸ 换平台/恢复链路 ◂

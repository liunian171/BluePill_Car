...
static speed_loop_cfg_t    s_cfg;
static speed_loop_io_t     s_io;
static PID_Handle          s_pid[SPEED_LOOP_MAX_CH];
static speed_loop_state_t  s_st[SPEED_LOOP_MAX_CH];
static int32_t             s_prev_enc[SPEED_LOOP_MAX_CH];
static uint32_t            s_prev_tick[SPEED_LOOP_MAX_CH];
static uint8_t             s_basis_ok[SPEED_LOOP_MAX_CH];   /* 测速基准是否已建立 */
static float               s_dt[SPEED_LOOP_MAX_CH];         /* 最近一次有效 dt（供 PID 复用） */
...
{
    return (s_inited && id < s_cfg.ch_count) ? 1 : 0;
}
...
{
    pid_reset(&s_pid[id]);
    if (s_io.brake) s_io.brake(id);
    s_st[id].target_rpm = 0.0f;
    s_st[id].actual_rpm = 0.0f;
    s_st[id].out_rpm    = 0.0f;
    s_st[id].running    = 0;
    s_basis_ok[id]      = 0;
}
...
speed_loop_ret_t speed_loop_init(const speed_loop_cfg_t *cfg, const speed_loop_io_t *io)
{
    ...
    for (uint8_t i = 0; i < s_cfg.ch_count; i++) {
        PID_Params_t p;
        p.kp = s_cfg.ch[i].kp;  p.ki = s_cfg.ch[i].ki;  p.kd = s_cfg.ch[i].kd;
        p.out_min = s_cfg.ch[i].out_min;
        p.out_max = s_cfg.ch[i].out_max;
        p.integral_limit = 0.0f;            /* 0 = 由 pid_init 取输出幅值一半（增量式不依赖 ∫） */
        pid_init(&s_pid[i], PID_MODE_INCREMENTAL, &p);

        s_st[i].target_rpm = 0.0f;
        s_st[i].actual_rpm = 0.0f;
        s_st[i].out_rpm    = 0.0f;
        s_st[i].running    = 0;
        s_prev_enc[i]      = 0;
        s_prev_tick[i]     = 0;
        s_basis_ok[i]      = 0;             /* 首次 measure 只建基准，不产出值 */
        s_dt[i]            = 0.0f;
    }
...
{
    if (!s_inited) return SPEED_LOOP_ERR_NOT_INIT;
    if (id >= s_cfg.ch_count) return SPEED_LOOP_ERR_BAD_ARG;

    int32_t enc = s_io.read_enc(id);

    /* 基准未建立：只建立基准并退出（等价于原实现"首帧 dt=0 丢弃"） */
    if (!s_basis_ok[id]) {
        s_prev_enc[id]  = enc;
        s_prev_tick[id] = now_ms;
        s_basis_ok[id]  = 1;
        return SPEED_LOOP_ERR_BAD_ARG;
    }

    float dt = (float)(now_ms - s_prev_tick[id]) * 0.001f;
    if (dt < DT_MIN_S || dt > DT_MAX_S) {
        s_prev_enc[id]  = enc;              /* 间隔异常：丢弃本次并重同步，不做除法 */
        s_prev_tick[id] = now_ms;
        return SPEED_LOOP_ERR_BAD_ARG;
    }

    int32_t delta = enc - s_prev_enc[id];
    if (s_cfg.ch[id].enc_span > 0) {        /* 编码器计数回绕校正（16 位定时器 = 65536） */
        int32_t half = s_cfg.ch[id].enc_span / 2;
        if (delta >  half) delta -= s_cfg.ch[id].enc_span;
        if (delta < -half) delta += s_cfg.ch[id].enc_span;
    }
    ...
    if (rpm_out) {
        *rpm_out = (float)delta * 60.0f / (dt * s_cfg.ch[id].ppr)
                   * (float)s_cfg.ch[id].fb_sign;
    }
...
speed_loop_ret_t speed_loop_resync(uint8_t id, uint32_t now_ms)
{
    ...
    s_prev_enc[id]  = s_io.read_enc(id);
    s_prev_tick[id] = now_ms;
    s_basis_ok[id]  = 1;
    return SPEED_LOOP_OK;
}
...
speed_loop_ret_t speed_loop_update(uint32_t now_ms)
{
    ...
    for (uint8_t m = 0; m < s_cfg.ch_count; m++) {
        const speed_loop_ch_cfg_t *c = &s_cfg.ch[m];
        float tgt = s_st[m].target_rpm;
        ...
        if (tgt < SPEED_LOOP_MIN_RUN_RPM && tgt > -SPEED_LOOP_MIN_RUN_RPM) {
            ...
            s_st[m].target_rpm = tgt;       /* 保留 |tgt|<1 的原始意图（遥测可读，非 0 也如实反映） */
            continue;
        }
        ...
        s_st[m].actual_rpm = rpm;

        /* 带符号前馈：本环工作域 = 带符号 RPM，方向由符号统一表达 */
        float ff  = tgt * c->ff_gain;
        float out = pid_update(&s_pid[m], tgt, rpm, s_dt[m]) + ff;
        out = clampf(out, c->out_min, c->out_max);

        if (s_io.set_rpm) s_io.set_rpm(m, out);
...
speed_loop_ret_t speed_loop_set_target(uint8_t id, float rpm_signed)
{
    ...
    s_st[id].target_rpm = rpm_signed;
    if (rpm_signed < SPEED_LOOP_MIN_RUN_RPM && rpm_signed > -SPEED_LOOP_MIN_RUN_RPM) {
        channel_park(id);
        s_st[id].target_rpm = rpm_signed;   /* channel_park 会清目标，这里还原原始意图 */
    }
    return SPEED_LOOP_OK;
}
...
speed_loop_ret_t speed_loop_set_gains(uint8_t id, float kp, float ki, float kd)
{
    ...
    s_cfg.ch[id].ki = ki;
    s_cfg.ch[id].kd = kd;
    pid_set_gains(&s_pid[id], kp, ki, kd);
    return SPEED_LOOP_OK;
}

...
static app_display_cfg_t g_cfg;
static app_display_io_t  g_io;
static uint8_t           g_inited = 0;

static uint32_t t_disp      = 0;
static uint8_t  g_disp_page = 0;

/* 页4/页6 数据源 (原 g_last_cmd/g_last_resp 收编) */
static char g_last_cmd[20]  = "NONE";
static char g_last_resp[20] = "-";

/* 带符号浮点 → 四舍五入 int（显示用; 已归拢 common/fmt.h → fmt_spd_to_int） */
...
{
    if (!g_inited) return;
    if (now - t_disp < g_cfg.disp_period_ms) return;
    t_disp = now;
    ...
    int32_t enc0 = g_io.enc_count(0, g_io.ctx);
    int32_t enc1 = g_io.enc_count(1, g_io.ctx);
    ...
    case 1: { /* 速度环 实际->目标 (带符号, 由执行组件状态读出) */
        ...
        snprintf(b,26,"M0:%d->%d  M1:%d->%d",
                 fmt_spd_to_int(s0.actual_rpm), (int)s0.target_rpm,
                 fmt_spd_to_int(s1.actual_rpm), (int)s1.target_rpm);
        page_line(1,b); break;
    }
    ...
    case 3: { /* 灰度 + IMU 状态
        ...
        uint8_t g1=g_io.gray_read(0, g_io.ctx), g2=g_io.gray_read(1, g_io.ctx);
        uint8_t g3=g_io.gray_read(2, g_io.ctx), g4=g_io.gray_read(3, g_io.ctx);
        uint8_t g5=g_io.gray_read(4, g_io.ctx);
        snprintf(b,26,"G:%d%d%d%d%d  %s",
                 g1?1:0,g2?1:0,g3?1:0,g4?1:0,g5?1:0, (prog<100)?"CAL":"OK");
        page_line(3,b); break;
    }
    case 4: /* 最近应答 (指令接收后的回复, 无线调试确认) */
        snprintf(b,26,"RSP:%-17s", g_last_resp);
        page_line(4,b); break;
    case 5: { /* PID 参数 M0/M1 (合并一行); 真值从组件读, ×100 显示 */
        ...
        snprintf(b,26,"P%d,%d I%d,%d D%d,%d",
                 fmt_spd_to_int(kp[0]*100.0f), fmt_spd_to_int(kp[1]*100.0f),
                 fmt_spd_to_int(ki[0]*100.0f), fmt_spd_to_int(ki[1]*100.0f),
                 fmt_spd_to_int(kd[0]*100.0f), fmt_spd_to_int(kd[1]*100.0f));
        page_line(5,b); break;
    }
    case 6: /* 最近执行的命令 (无线命令执行确认) */
        snprintf(b,26,"CMD:%-16s", g_last_cmd);
        page_line(6,b); break;
              ...
              *       / 末列 = UART 静默秒数 或 FZ=OLED 已熔断停刷 */
    {
        ...
        uint8_t usb_on = g_cfg.usb_enabled ? g_io.usb_on(g_io.ctx) : 0u;
        /* UART: MCU 无法感知 SPP 连接态, 显示距最近收字节的秒数 (从未收过显 "-") */
        char us[10];
        uint32_t sil = g_io.uart_silence_s(g_io.ctx);
        if (sil != 0xFFFFFFFFu)
...
void app_display_note_cmd(const char *s, void *ctx)
{
    ...
    if (s == NULL) return;
    size_t n = strlen(s);
    if (n > sizeof(g_last_cmd) - 1) n = sizeof(g_last_cmd) - 1;
    memcpy(g_last_cmd, s, n);
    g_last_cmd[n] = '\0';
}
...
void app_display_note_resp(const char *s, void *ctx)
{
    ...
    if (s == NULL) return;
    size_t n = strlen(s);
    if (n > sizeof(g_last_resp) - 1) n = sizeof(g_last_resp) - 1;

...
| `doc/架构评审文档.md` | 耦合性/可配置性评审快照 + 治理优先级（C1-C8 / D1-D3） | ⚠️ 评审后代码未动（学习思考期），发现项仍有效 |
| `doc/代码风格与模块衔接指南.md` | 代码形态规范：分层性质/命名注释/机制规则/新模块衔接义务 8 项 | ✅ 新确立条款（🔶 标注） |
| `doc/结构优化顺序分析.md` | 治理顺序推演（契约先行 / 实现后搬）+ 学习期进度登记 | ⚠️ 当前为学习思考期，未启动实施 |
...
| D3 | `pwm.h` 预留未用结构 + 策略头 include CubeMX `tim.h`（C6） | ⬜ 待治理期处理 |
| C1 | `uart_cmd_parser.c` 死代码仍在编译（main.c 内联分发为唯一实现） | ⬜ 待治理期处理 |
| C2 | `line_follower.c` 直调 `HAL_GPIO_ReadPin`，PC 桩不可编译 | ⬜ 待治理期处理 |

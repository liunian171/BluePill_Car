<!-- 编码: UTF-8 -->
# 舵机旧设计稿（归档）— 源自上游驱动工程 A_board_pwm_driver_test

> 归档时间: 2026-09-11 | 来源: 上游 `A_board_pwm_driver_test/Core/Src/driver/servo.cpp` 头部 1~172 行（对应上游 `Servo_Driver_Design.md`）
> 归档原因: 该段是上游工程的**设计草案**（规划中、未实现），随移植整文件复制进本仓库 `servo.cpp`，与同文件真实实现（原 173 行起）互相矛盾。已从代码中剥离，**内容原样保留于此备查**。
> 状态: 只读归档，不维护、不实施。
> 重要: 文中提到的文件与类 —— `servo_core.cpp` / `servo_core.h` / `ServoUART` / `ServoI2C` / `PCA9685` / `app/main.cpp` —— **在本仓库与上游工程中都不存在**，仅出现在本稿与上游 Servo_Driver_Design.md 的规划里。
> 溯源证据: 剥离线前，本仓库 `servo.cpp` 与上游 `servo.cpp` 全文仅差 1 行 —— `max_angle(180)` → `max_angle(270)`（本项目换 270° 舵机）。其余字节级一致。

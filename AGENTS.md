...
| `doc/调参工具链规划.md` | 闭环调参工具链现状盘点 + 硬约束（带宽/阻塞计算）+ 三步走路线 + 4 项待决策 |
...
**当前主线（用户口径 2026-09-12：先做完转向 + 闭环，巡线后置）**
- [ ] **① C2 抽 `speed_loop` 执行组件**（= "闭环"，与 `steering` 同构：纯 C + 输出注入 + tick 外部化 + PC 桩）← **最高优先**
- [ ] ② C1 桥接层家族契约（错误通道形态 / init 签名 / id+handle 校验 / oled 归一 / 极性上提）
- [ ] ③ 死代码清理 + 自查项 N1~N3（见下"代码卫生"）
- [ ] ④ 硬件验证补测（示波器 PB8 脉宽 / 舵机物理动作目视）
- 分层盘点与逐层证据见 `doc/结构优化顺序分析.md` §10（含各策略层 `TODO/占位/预留` 扫描结果）

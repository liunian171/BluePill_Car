...
- [ ] **外设故障不得拖垮系统节拍（系统级完善，2026-09-19 登记 → 后置解决）**：与外设共处主循环的阻塞访问必须①失败代价按控制节拍预算（超时 ≤20ms 量级、重试次数有界）②沉默失败必须可见（计数可查）③辅助通道带熔断或移出控制节拍。**来源事件**=OLED 写超时 100ms×排线接触不良 → pass 锁死 100ms → 速度环减半（专案已结案；OLED 已按①②落地补丁）。**未治理项**：UART 无 `HAL_UART_ErrorCallback`、`ack()/DUMP/tx_raw` 阻塞发送、`usb_send_raw` USBD_BUSY 静默丢弃 —— 随 P3（瘦身/重构）批次统一处理，**不做逐个外设散修**。详 `doc/开发跟踪.md` 设计决议 2026-09-19 + `doc/节拍拉长与I2C总线专案.md`

...
| `doc/架构符合性审计_2026-09-20.md` | **代码 vs 设计文档符合性审计**（分层依赖/义务 8 项/时序/死代码纪律；P1/P2 分级 + 机械核查命令可复现；整改批次已登记开发跟踪） |
...
| `doc/发送路径专项设计.md` | **R3 发送路径专项（2026-09-21）**：统一 tx_stream 队列 + 分段异步发送 + 失败/丢弃计数可见 + UART ErrorCallback 重挂 + DUMP 非阻塞流式 |

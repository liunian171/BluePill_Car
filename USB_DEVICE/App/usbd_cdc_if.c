...
static int8_t CDC_Control_FS(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
  ...
  /*******************************************************************************/
    ...
    case CDC_SET_CONTROL_LINE_STATE:
      /* [P2 双链路仲裁] wLength=0 类请求: pbuf = USBD_SetupReqTypedef 指针,
       * DTR 位在 wValue 低字节 bit0 (pbuf[2]); 见 usbd_cdc.c CDC_Setup 分发 */
      g_usb_dtr = (pbuf[2] & 0x01U) ? 1U : 0U;
    break;
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  /* USER CODE BEGIN 6 */
#ifdef USB_ECHO_TEST
  /* 独立链路测试 (中断上下文): PING -> PONG, 其余字节原样回显。
   * 回显先拷到 UserTxBufferFS 再发, 避免发送未完成时 Rx 重挂同缓冲被改写。
   * CDC_Transmit_FS 非阻塞, 忙时返回 USBD_BUSY (测试低速率下可忽略)。 */
  if (*Len >= 4 && Buf[0] == 'P' && Buf[1] == 'I' && Buf[2] == 'N' && Buf[3] == 'G') {
    CDC_Transmit_FS((uint8_t *)"PONG\r\n", 6);
  } else if (*Len > 0 && *Len <= APP_TX_DATA_SIZE) {
